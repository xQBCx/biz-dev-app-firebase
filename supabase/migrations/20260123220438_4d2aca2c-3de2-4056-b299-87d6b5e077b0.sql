...
BEGIN
  ...
  -- CRITICAL: Check for approved access request with pre-set permissions
  BEGIN
    ...
    IF v_access_request.id IS NOT NULL AND v_access_request.default_permissions IS NOT NULL THEN
      ...
      -- Loop through each module in the default_permissions JSON
      ...
      LOOP
        BEGIN
          ...
          ON CONFLICT (user_id, module) DO UPDATE SET
            can_view = COALESCE((v_perm_obj->>'can_view')::boolean, false),
            can_create = COALESCE((v_perm_obj->>'can_create')::boolean, false),
            can_edit = COALESCE((v_perm_obj->>'can_edit')::boolean, false),
            can_delete = COALESCE((v_perm_obj->>'can_delete')::boolean, false);
        EXCEPTION WHEN OTHERS THEN
          -- Skip invalid module names gracefully
          RAISE LOG 'handle_new_user: Skipped invalid module % for user %: %', v_module_key, NEW.id, SQLERRM;
        END;
      END LOOP;
      
      RAISE LOG 'handle_new_user: Applied % permissions from access request for user %', 
        jsonb_object_keys(v_access_request.default_permissions::jsonb), NEW.id;
    END IF;
  EXCEPTION WHEN OTHERS THEN

...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Continuous zoom effect - camera moves forward
    float zoomOffset = mod(iTime * uZoomSpeed, 4.0);
    
    // Slight downward tilt for immersive club floor perspective
    float immersiveTilt = -0.15;
    p.y += immersiveTilt;

    vec3 ro = vec3(0.0, -0.1, -zoomOffset);
  ...
  // Club strobe/pulse lighting effect
  float strobePulse = sin(iTime * 3.0) * 0.5 + 0.5;
  float strobeFlash = pow(strobePulse, 4.0) * 0.3;
  vec3 strobeColor = uScanColor * strobeFlash * fade;
  
  // Depth-based atmosphere glow (fog-like club atmosphere)
  float atmosphereGlow = exp(-dist * 0.8) * 0.15;
  vec3 atmosphereColor = mix(uLinesColor, uScanColor, 0.3) * atmosphereGlow;

    color = gridCol + scanCol + scanAura + strobeColor + atmosphereColor;

  float n = fract(sin(dot(gl_FragCoord.xy + vec2(iTime * 123.4), vec2(12.9898,78.233))) * 43758.5453123);
  color += (n - 0.5) * uNoise;
  color = clamp(color, 0.0, 1.0);
  float alpha = clamp(max(lineVis, max(combinedPulse, atmosphereGlow)), 0.0, 1.0);
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        try {
          await (DeviceOrientationEvent.requestPermission as () => Promise<PermissionState>)();
        } catch {
          // noop
        }
      }

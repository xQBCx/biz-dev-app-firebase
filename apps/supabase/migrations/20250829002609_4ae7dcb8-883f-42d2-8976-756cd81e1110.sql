...
('Guest Service Excellence', 'Master the art of exceptional guest service and create memorable experiences for every visitor.', 'Front Desk', 'beginner', 45, 100, 'Sarah Johnson', 
 ...
 ARRAY[], ARRAY['customer service', 'communication', 'hospitality'], true),
...
('Room Cleaning Standards', 'Learn the comprehensive standards for maintaining pristine guest rooms and common areas.', 'Housekeeping', 'beginner', 40, 80, 'Maria Rodriguez',
 ...
 ARRAY[], ARRAY['cleaning', 'standards', 'quality'], true),
...
('Preventive Maintenance Fundamentals', 'Essential preventive maintenance practices to keep properties running smoothly and efficiently.', 'Maintenance', 'beginner', 50, 110, 'Robert Torres',
 ...
 ARRAY[], ARRAY['maintenance', 'prevention', 'equipment'], true),
...
('Revenue Management Basics', 'Introduction to revenue optimization strategies and pricing techniques for maximum profitability.', 'Operations', 'intermediate', 55, 130, 'Amanda Foster',
 ...
 ARRAY[], ARRAY['revenue', 'pricing', 'analytics'], true),
...
('Leadership & Team Development', 'Essential leadership skills for managers and supervisors to build and motivate high-performing teams.', 'Operations', 'advanced', 70, 180, 'James Peterson',
 ...
 ARRAY[], ARRAY['leadership', 'management', 'teams'], true),
...
('Workplace Safety & OSHA Compliance', 'Comprehensive workplace safety training covering OSHA requirements and best practices.', 'General', 'beginner', 45, 100, 'Lisa Thompson',
 ...
 ARRAY[], ARRAY['safety', 'osha', 'compliance'], true),
...
('Customer Experience Excellence', 'Advanced strategies for creating exceptional customer experiences across all touchpoints.', 'General', 'intermediate', 65, 140, 'Carlos Martinez',
 ...
 ARRAY[], ARRAY['customer experience', 'service', 'loyalty'], true);

-- Insert sample modules for courses
INSERT INTO public.academy_course_modules (course_id, title, description, content_type, duration_minutes, xp_reward, order_index) 
SELECT c.id, 
       CASE 
         WHEN c.title = 'Guest Service Excellence' THEN 
           CASE ROW_NUMBER() OVER (PARTITION BY c.id ORDER BY random())
             WHEN 1 THEN 'Introduction to Hospitality'
             WHEN 2 THEN 'First Impressions Matter'
             WHEN 3 THEN 'Active Listening Skills'
             WHEN 4 THEN 'Handling Difficult Guests'
             WHEN 5 THEN 'Follow-up and Follow-through'
           END
         WHEN c.title = 'Room Cleaning Standards' THEN
           CASE ROW_NUMBER() OVER (PARTITION BY c.id ORDER BY random())
             WHEN 1 THEN 'Bathroom Deep Clean'
             WHEN 2 THEN 'Bedroom Setup & Linens'
             WHEN 3 THEN 'Common Area Maintenance'
             WHEN 4 THEN 'Quality Inspections'
           END
         ELSE 'Module ' || ROW_NUMBER() OVER (PARTITION BY c.id ORDER BY random())
       END as title,
       'Detailed training module covering key concepts and practical applications.' as description,
       'video' as content_type,
       15 as duration_minutes,
       25 as xp_reward,
       ROW_NUMBER() OVER (PARTITION BY c.id ORDER BY random()) as order_index
FROM public.academy_courses c
CROSS JOIN generate_series(1, 
  CASE 
    WHEN c.title = 'Guest Service Excellence' THEN 5
    WHEN c.title = 'Room Cleaning Standards' THEN 4  
    ELSE 3
  END
) series;
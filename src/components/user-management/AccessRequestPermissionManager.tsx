...
interface AccessRequestPermissionManagerProps {
  requestId: string;
...
export const AccessRequestPermissionManager = ({ requestId, requesterEmail }: AccessRequestPermissionManagerProps) => {
  ...
  const savePermissions = async (newPermissions: Record<string, any>) => {
    try {
      const { error } = await supabase
        .from('access_requests')
        .update({ default_permissions: newPermissions as Json })
        .eq('id', requestId);
    ...
    <div className="space-y-6">
      <div className="p-4 bg-muted rounded-lg">
        <p className="text-sm text-muted-foreground">
          Setting permissions for: <strong>{requesterEmail}</strong>
        </p>
        <p className="text-xs text-muted-foreground mt-1">
          These permissions will be applied when the user's access request is approved and they create their account.
        </p>
      </div>

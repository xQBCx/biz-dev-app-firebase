...
import { initializeStripeConnect } from "@/lib/stripe-connect";
...
export function StripeConnectOnboarding() {
  ...
  const [stripeConnectInstance, setStripeConnectInstance] = useState<ReturnType<typeof initializeStripeConnect> | null>(null);

  // Check for return from Stripe (legacy - keeping for backwards compatibility)
  useEffect(() => {
  ...
  const handleConnect = async () => {
    try {
      ...
      const instance = initializeStripeConnect(data.clientSecret);
  ...
  const handleOnboardingExit = () => {
    ...
    refetchStatus();
    toast.success("Onboarding session ended", {
          ...
          {isFullyConnected ? (
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground text-center">
                Takes about 5 minutes. All verification happens right here.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={showOnboarding} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" hideCloseButton>
          <DialogHeader>
            ...
            <DialogDescription>
              Complete the verification below to enable fast bank transfers
            </DialogDescription>
          </DialogHeader>

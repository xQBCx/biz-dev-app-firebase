...
const supabase = createClient(supabaseUrl, serviceKey, { fetch: (url, options) => fetch(url, { ...options, timeout: 120000 }) });
...
Attempting to import data for table: ${tableName}`);
    ...
    try {
        ...
        const batchSize = tableName === 'ai_messages' ? 50 : 500;

        for (let i = 0; i < data.length; i += batchSize) {
            const batch = data.slice(i, i + batchSize);
            const { error } = await supabase.from(tableName).upsert(batch, { onConflict: onConflictColumn, ignoreDuplicates: false });
            if (error) {
                console.error(`ERROR for table ${tableName} (batch ${Math.floor(i / batchSize) + 1}):`, error.details || error.message);
                return;
            }
        }
